...
<body class="m-0 h-full" style="background-color: #f7f0e9;">
  ...
  <!-- videoIds 슬라이더 -->
  <div class="video-slider-wrapper">
    <h2 style="font-size: 2em; font-weight: bold; color: black; text-align: left; margin-left: 1%;">${rq.loginedTeam} 선수들 근황</h2>

    <c:choose>
      <c:when test="${not empty videoIds}">
        <button class="btn-slide btn-prev" onclick="slideVideosById('videoSlider1', -1)"><i class="fas fa-chevron-left"></i></button>
        <button class="btn-slide btn-next" onclick="slideVideosById('videoSlider1', 1)"><i class="fas fa-chevron-right"></i></button>

        <div class="video-slider" id="videoSlider1">
          <c:forEach var="id" items="${videoIds}">
            <div class="video-item">
      ...
      <c:otherwise>
        <p>임베드할 주요 영상이 없습니다.</p>
      </c:otherwise>
    </c:choose>
  </div>
  
    <div class="video-slider-wrapper">
    <h2 style="font-size: 2em; font-weight: bold; color: black; text-align: left; margin-left: 1%;">KBO 웃긴 영상</h2>

    <c:choose>
      <c:when test="${not empty videoIds2}">
        <button class="btn-slide btn-prev" onclick="slideVideosById('videoSlider2', -1)"><i class="fas fa-chevron-left"></i></button>
        <button class="btn-slide btn-next" onclick="slideVideosById('videoSlider2', 1)"><i class="fas fa-chevron-right"></i></button>

        <div class="video-slider" id="videoSlider2">
          <c:forEach var="id" items="${videoIds2}">
            <div class="video-item">
              <iframe src="https://www.youtube.com/embed/${id}" frameborder="0" allowfullscreen></iframe>
            </div>
          </c:forEach>
